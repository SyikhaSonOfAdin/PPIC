...
import { SAP } from "../_interface/sap.interface";
import sapDummyServices from "../services/sap-dummy";

const sapDummyController = {
  get: {
    by: {
      projectNo: async (req: Request, res: Response) => {
        const stages = [
          "request_purchase",
          "purchase_order",
    ...
    status: {
      projectNo: async (req: Request, res: Response) => {
        ...
        try {
          ...
          const response = await sapDummyServices.get.status.projectNo(
            projectNo,
            //@ts-ignore
            s,
          );
          if (response.success) {
            ...
            stages.forEach((s) => {
              //@ts-ignore
              response.data.data[s].forEach((g: SAP) => {
                items.set(
                  g.item_code + "|" + g.group + "|" + g.item_description,
                  {
                    request_purchase: 0,
                    purchase_order: 0,
             ...
             */
            Array.from(items.keys()).forEach((key: string) => {
              ...
              const sum = {
                ...
                description,
                request_purchase:
                  response.data.data.request_purchase?.reduce(
                    (acc, row) =>
                      row.item_code === id ? acc + (row.quantity ?? 0) : acc,
                    0,
                  ) || 0,
                purchase_order:
                  response.data.data.purchase_order?.reduce(
                    (acc, row) =>
                      row.item_code === id ? acc + (row.quantity ?? 0) : acc,
                    0,
                  ) || 0,
                goods_receive_purchase_order:
                  response.data.data.goods_receive_purchase_order?.reduce(
                    (acc, row) =>
                      row.item_code === id ? acc + (row.quantity ?? 0) : acc,
                    0,
                  ) || 0,
                inventory_transfer_request:
                  response.data.data.inventory_transfer_request?.reduce(
                    (acc, row) =>
                      row.item_code === id ? acc + (row.quantity ?? 0) : acc,
                    0,
                  ) || 0,
                inventory_transfer:
                  response.data.data.inventory_transfer?.reduce(
                    (acc, row) =>
                      row.item_code === id ? acc + (row.quantity ?? 0) : acc,
                    0,
                  ) || 0,
                goods_issue:
                  response.data.data.goods_issue?.reduce(
                    (acc, row) =>
                      row.item_code === id ? acc + (row.quantity ?? 0) : acc,
                    0,
                  ) || 0,
                goods_receipt:
                  response.data.data.goods_receipt?.reduce(
                    (acc, row) =>
                      row.item_code === id ? acc + (row.quantity ?? 0) : acc,
                    0,
                  ) || 0,
                   ...
                   */
                request_purchase_kbn:
                  response.data.data.request_purchase_kbn?.reduce(
                    (acc, row) =>
                      row.item_code === id ? acc + (row.quantity ?? 0) : acc,
                    0,
                  ) || 0,
                purchase_order_kbn:
                  response.data.data.purchase_order_kbn?.reduce(
                    (acc, row) =>
                      row.item_code === id ? acc + (row.quantity ?? 0) : acc,
                    0,
                  ) || 0,
                goods_receive_purchase_order_kbn:
                  response.data.data.goods_receive_purchase_order_kbn?.reduce(
                    (acc, row) =>
                      row.item_code === id ? acc + (row.quantity ?? 0) : acc,
                    0,
                  ) || 0,
                inventory_transfer_request_kbn:
                  response.data.data.inventory_transfer_request_kbn?.reduce(
                    (acc, row) =>
                      row.item_code === id ? acc + (row.quantity ?? 0) : acc,
                    0,
                  ) || 0,
                inventory_transfer_kbn:
                  response.data.data.inventory_transfer_kbn?.reduce(
                    (acc, row) =>
                      row.item_code === id ? acc + (row.quantity ?? 0) : acc,
                    0,
                  ) || 0,
                goods_issue_kbn:
                  response.data.data.goods_issue_kbn?.reduce(
                    (acc, row) =>
                      row.item_code === id ? acc + (row.quantity ?? 0) : acc,
                    0,
                  ) || 0,
                goods_receipt_kbn:
                  response.data.data.goods_receipt_kbn?.reduce(
                    (acc, row) =>
                      row.item_code === id ? acc + (row.quantity ?? 0) : acc,
                    0,
                  ) || 0,
