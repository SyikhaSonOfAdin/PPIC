...
const projectControllers = {
  add: async (req, res, next) => {
    ...
    try {
      ...
      try {
        ...
        const projectId = await projectServices.add(
          companyId,
          categoryId,
          projectNo,
          client,
  ...
  copy: async (req, res, next) => {
    ...
    try {
      ...
      try {
        await connection.beginTransaction();
        const project = await projectServices.get.onlyOne(
          projectId,
          connection,
        );
        const projectDetail = await projectDetailServices.get(
          projectId,
          connection,
        );
        ...
        const id = await projectServices.add(
          project.COMPANY_ID,
          project.CATEGORY_ID,
          project.PROJECT_NO + " Copy",
          project.CLIENT,
          userId,
        );
        await projectDetailServices.add(
    ...
    all: async (req, res, next) => {
      ...
      try {
        ...
        try {
          ...
          await projectDetailServices.edit.all(
            {
              projectId,
              userId,
              name,
  ...
  get: {
    all: async (req, res, next) => {
      ...
      try {
        const connection = await PPIC.getConnection();
        const [periodYear] = await connection.query(
          othersQuerys.select.distinct.period.year,
          [companyId, companyId],
        );
        try {
          await connection.beginTransaction();
          ...
          const projects = (
            await Promise.all(
              data.map(async (item) => {
                const actual = await actualServices.get.all(
                  item.ID,
                  connection,
                );
                ...
                if (m && y) {
                  ...
                  const plans = await plansServices.get.all(
                    item.ID,
                    connection,
                  );
                  ...
                  actual.reduce((sum, i) => {
                    if (Number(i.PERIOD_YEAR) == y) {
                      if (Number(i.PERIOD_MONTH.split("-")[0], 10) <= m) {
                        if (
                          new Date(
                            Number(i.PERIOD_YEAR),
                            Number(i.PERIOD_MONTH.split("-")[0]) - 1,
                            Number(i.PERIOD_MONTH.split("-")[1]),
                    ...
                    return {
                      ...
                      progress: {
                        PERCENTAGE:
                          totalActual > 0
                            ? (
                                (totalActual / parseFloat(item.CAPACITY)) *
                                100
                              ).toFixed(2) + "%"
                            : "0%",
                        ...
                        DEVIATION:
                          (
                            realActual.toFixed(2) - tempPlans.toFixed(2)
                          ).toFixed(2) + "%",
                      },
                      actual: actual,
                    };
    ...
    summary: async (req, res, next) => {
      ...
      try {
        ...
        try {
          ...
          const categories = await categoryServices.get.all(
            companyId,
            connection,
          );
          ...
          const groupedProjects = await Promise.all(
            categories.map(async (category) => {
              ...
              const workLoad = await workLoadServices.get.onlyOne.byYear(
                category.ID,
                year,
                connection,
              );

              const projectsAtributes = await projectServices.get.byCategoryId(
                category.ID,
                connection,
              );
              ...
              plansData.flat().forEach((eachData) => {
                const monthIndex =
                  Number(eachData.PERIOD_MONTH.split("-")[0], 10) - 1;
                if (
                  monthIndex >= 0 &&
                  monthIndex < months.length &&
                  eachData.PERIOD_YEAR == year
                ) {
                  periods.add(
                    `${eachData.PERIOD_YEAR} / ${months[monthIndex]}`,
                  );
                }
              });
              ...
              return {
                ...
                PLANS: uniqueLabels.map((label) => {
                  ...
                  const planItem = plansData
                    .flat()
                    .filter(
                      (item) =>
                        item.PERIOD_YEAR == year &&
                        months[
                          Number(item.PERIOD_MONTH.split("-")[0], 10) - 1
                        ] === month,
                    );
                  ...
                  return {
                    x: label,
                    y: (totalAmount ?? 0).toFixed(2),
                  };
                }),
                ACTUAL: uniqueLabels.map((label) => {
                  const [year, month] = label.split(" / ");

                  const actualItem = actualData
                    .flat()
                    .filter(
                      (item) =>
                        item.PERIOD_YEAR == year &&
    ...
    download: {
      plans_and_actual: async (req, res) => {
        ...
        if (!companyId || !year || !month_start || !month_end)
          return res.status(400).json({ message: "Invalid Parameter" });
        try {
          const CONNECTION = await PPIC.getConnection();
          try {
            const [actual] = await CONNECTION.query(
              othersQuerys.select.actual.by.period.month,
              [companyId, year, month_start, month_end],
            );
            const [plans] = await CONNECTION.query(
              othersQuerys.select.plan.by.period.month,
              [companyId, year, month_start, month_end],
            );

            const temp = new Map(
              [...plans, ...actual].map((item) => [
                item.PROJECT_ID + "|" + item.YEAR.toString() + "|" + item.MONTH,
                {
            ...
            const pd = await Promise.all(
              Array.from(temp.values()).map(async (p) => {
                const project = await projectServices.get.onlyOne(
                  p.PROJECT_ID,
                  CONNECTION,
                );
            ...
            const data = Array.from(temp.values()).map((p) => {
              return {
                ...
                TOTAL_AMOUNT_PLAN:
                  plans.find(
                    (item) =>
                      item.PROJECT_ID == p.PROJECT_ID && item.MONTH == p.MONTH,
                  )?.TOTAL_AMOUNT_PLAN ?? 0,
                TOTAL_AMOUNT_ACTUAL:
                  actual.find(
                    (item) =>
