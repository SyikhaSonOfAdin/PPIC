...
const fieldLabel = (text: string) =>
  `<p style="margin:0 0 4px;font-size:10px;font-weight:600;color:#94a3b8;
    text-transform:uppercase;letter-spacing:.5px;line-height:14px;">${text}</p>`;

/** Badge status remark */
const remarkBadge = (status: string) => {
...
const projectBadge = (status: string) => {
 ...
 */
const buildRemarkCard = (r: RemarkRow, isLast: boolean): string => {
...
const buildProjectCard = (group: ProjectGroup, appUrl: string): string => {
...
export const emailServices = {
  ...
  template: {
    projectRemark: (
      ...
      const grouped = safeData.reduce<Record<string, ProjectGroup>>(
        (acc, item) => {
          if (!acc[item.ID]) {
            acc[item.ID] = {
              project_id: item.ID,
