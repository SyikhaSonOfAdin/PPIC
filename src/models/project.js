...
const QUERY = {
    ...
    select: {
        ...
        DATE_FORMAT(CP.${table.COLUMN.INPUT_DATE}, '%Y-%m-%d') AS INPUT_DATE, 
        C.${categoryTable.COLUMN.ID} AS CATEGORY_ID, 
        C.${categoryTable.COLUMN.UOM}, 
        C.${categoryTable.COLUMN.NAME} AS CATEGORY_NAME 
        FROM ${table.TABLE} AS CP 
        JOIN ${categoryTable.TABLE} AS C 
        ON CP.${table.COLUMN.CATEGORY_ID} = C.${categoryTable.COLUMN.ID} 
        JOIN ${userTable.TABLE} AS U 
        ON CP.${table.COLUMN.INPUT_BY} = U.${userTable.COLUMN.ID} 
        WHERE CP.${table.COLUMN.ID} = ?`,
